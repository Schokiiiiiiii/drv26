...
volatile uint8_t* memory_cursor;  // aims at the start of the memory
char hex_letters[6] = {' ', ' ', ' ', ' ', ' ', ' '};
...
uint32_t read_keys(void) {
    const volatile uint32_t *keys = (volatile uint32_t *)(memory_cursor + KEYS_OFFSET);
    return *keys;
}
