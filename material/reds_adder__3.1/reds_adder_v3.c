...
{
	/*
	 * sysfs_emit is the function that should be used to write to the sysfs buffer.
	 * It acts like scnprintf, but already aware of the buffer size (== PAGE_SIZE)
	 */
			       ...
			       size_t count)
{
	...
	if (mutex_trylock(&priv->read_mutex)) {
		...
		if (strncmp(buf, "encrypt", strlen("encrypt")) == 0) {
			priv->encrypt = true;
		} else if (strncmp(buf, "decrypt", strlen("decrypt")) == 0) {
			priv->encrypt = false;
		} else {
			dev_err(priv->dev, "Invalid operation requested!\n");
			return -EINVAL;
		}
		mutex_unlock(&priv->read_mutex);
	} else {
			      ...
			      char *buf)
{
	 ...
	 */
	/*
	 * As a bonus, we could easily replace the three show() operations with a macro.
	 * With just three functions it is not worth the effort, but I have once written
	 * a driver with several dozen values to be put in sysfs, and explicitly
