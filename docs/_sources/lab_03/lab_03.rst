...
C'est d'ailleurs tout ce qu'il y a besoin d'être précisé dans le :file:`Makefile`,
...
Ce log, stocké dans :file:`/var/log/kern.log` ou :file:`/var/log/messages` sur la carte, peut être affiché directement via
...
des lignes ajoutées à sa fin (donc, sur la carte, :bash:`tail -f /var/log/messages` étant plus ou moins équivalent à :bash:`dmesg -w`).
...
.. admonition:: **Exercice 5 : accumulate module**
